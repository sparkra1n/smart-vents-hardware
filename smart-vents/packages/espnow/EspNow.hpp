...
class EspNow
{
    ...
    public:
        ...
        */
        [[__nodiscard__]] static esp_err_t send(const MacAddress& mac, const Message& msg, uint8_t count=1) { return get().sendI(mac, msg, count); }
        
        /**
         * @brief add peer to peer list in order to send message to it
         * @param mac peer's mac address
        */
        [[__nodiscard__]] static esp_err_t addPeer(const MacAddress& mac) { return get().addPeerI(mac); }

        /**
         * @brief remove peer from peer list in order make space in the list's buffer 
         * @param mac peer's mac address
        */
        [[__nodiscard__]] static esp_err_t remPeer(const MacAddress& mac) { return get().remPeerI(mac); }
        ...
        */
       [[__nodiscard__]] static bool read(MacAddress& mac, Message& msg) { return get().readI(mac, msg); }
};
